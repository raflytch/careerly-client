import { Link } from "react-router";
import { Button } from "@/components/ui/button";
import {
  Card,
  CardContent,
  CardDescription,
  CardHeader,
  CardTitle,
} from "@/components/ui/card";
import { Separator } from "@/components/ui/separator";
import { API_BASE_URL } from "@/config/axios";
import { FileText, FileSearch, BrainCircuit } from "lucide-react";

export default function LoginPage() {
  const handleGoogleLogin = () => {
    window.location.href = `${API_BASE_URL}/auth/google/login`;
  };

  return (
    <div className="flex min-h-screen">
      <div className="hidden flex-1 flex-col justify-between border-r border-border bg-muted/30 p-10 lg:flex">
        <span className="text-xl font-bold">Careerly</span>
        <div className="space-y-6">
          <h2 className="text-3xl font-bold leading-tight tracking-tight">
            Persiapkan karirmu
            <br />
            dengan lebih percaya diri
          </h2>
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              <div className="flex size-9 items-center justify-center rounded-lg bg-primary/10">
                <FileText className="size-4 text-primary" />
              </div>
              <div>
                <p className="text-sm font-medium">Resume Builder</p>
                <p className="text-xs text-muted-foreground">
                  Buat resume profesional dengan mudah
                </p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <div className="flex size-9 items-center justify-center rounded-lg bg-primary/10">
                <FileSearch className="size-4 text-primary" />
              </div>
              <div>
                <p className="text-sm font-medium">ATS Review</p>
                <p className="text-xs text-muted-foreground">
                  Pastikan resume lolos sistem ATS
                </p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <div className="flex size-9 items-center justify-center rounded-lg bg-primary/10">
                <BrainCircuit className="size-4 text-primary" />
              </div>
              <div>
                <p className="text-sm font-medium">Interview AI</p>
                <p className="text-xs text-muted-foreground">
                  Latihan interview dengan kecerdasan buatan
                </p>
              </div>
            </div>
          </div>
        </div>
        <p className="text-xs text-muted-foreground">
          © {new Date().getFullYear()} Careerly
        </p>
      </div>

      <div className="flex flex-1 items-center justify-center px-4">
        <Card className="w-full max-w-sm border-0 shadow-none lg:border lg:shadow-none">
          <CardHeader className="text-center">
            <div className="mb-2 text-2xl font-bold lg:hidden">Careerly</div>
            <CardTitle className="text-xl">Selamat datang</CardTitle>
            <CardDescription>
              Masuk untuk mulai mempersiapkan karir terbaikmu
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <Button
              variant="outline"
              className="w-full gap-3"
              onClick={handleGoogleLogin}
            >
              <svg viewBox="0 0 24 24" className="size-5">
                <path
                  d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z"
                  fill="#4285F4"
                />
                <path
                  d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                  fill="#34A853"
                />
                <path
                  d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
                  fill="#FBBC05"
                />
                <path
                  d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                  fill="#EA4335"
                />
              </svg>
              Masuk dengan Google
            </Button>
            <Separator />
            <p className="text-center text-xs text-muted-foreground">
              Akun terhapus?{" "}
              <Link
                to="/restore"
                className="font-medium text-foreground underline-offset-4 hover:underline"
              >
                Restore di sini
              </Link>
            </p>
          </CardContent>
        </Card>
      </div>
    </div>
  );
}
